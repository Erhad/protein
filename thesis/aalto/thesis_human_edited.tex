...
\thispagestyle{empty}
The core question of this thesis: given a goal to optimize some protein, what kind of methods should we utilize to get the best one

% #region [DRAFT] AI-generated intro snippet
AI GEN INTRO
...
for two reasons: (i) the map $\varphi$between sequences X= {x1,$\cdot$$\cdot$$\cdot$
,xn}and their biological function
y= {y1,$\cdot$$\cdot$$\cdot$,yn}is non-convex and (ii) has sparse support in the space of possible sequences
AL, which also grows exponentially in the length of the sequence Lfor alphabet A. This map $\varphi$is
...
$*$equal contribution, †correspondence: sam.sinai@dynotx.com, eric.kelsic@dynotx.com
Preprint. Work in progress.
Recent advances in DNA sequencing and synthesis technologies allow large assays which query
yfor specific sequences x with up to 105 physical samples per batch (Barrera et al., 2016). This
development presents an opening for machine learning to contribute in building better surrogate
models $\varphi$$'$: X $\to$y which approximate the oracle $\varphi$that maps each sequence to its true function. We
may use these models $\varphi$$'$as proxies of $\varphi$, in order to generate and screen sequences in silico before
they are sent to synthesis (Yang et al., 2019; Fox et al., 2007). While a large body of work has focused
on building better local approximate models $\varphi$$'$on already published data (Otwinowski et al., 2018;
...
performed. Hence data is often collected in serial batches bi, comprising data Dt = {b0,$\cdot$$\cdot$$\cdot$,bt}and
the problem of sequence design is generally cast as that of proposing batches so that we may find the
optimal sequence x$*$
t = argmaxx$\in$Dt $\varphi$(x) over the course of these experiments
% #endregion
----------------------------------
% #region [DRAFT] Planning notes and literature snippets
PLAN and thinking

----
Introduction
SNIPPET Protein optimization and design. Approaches can broadly be categorized using sequence, structure
...
Concurrently, Emami et al. (2023) used GWG for protein optimization with a prod

----
MLDE 
Act 1: The Problem is Established (1994--2016)
The story starts in the wet lab. Frances Arnold and others figure out that you don't need to understand how a protein works to improve it --- you just need a way to screen many variants and select the best ones. That's directed evolution, and it works. But it's slow, it gets stuck at local optima, and it burns through lab budget one variant at a time.
The field then asks: what does the fitness landscape actually look like? Two papers answer this by measuring it exhaustively. Wu et al. (2016) builds the GB1 landscape --- all 160,000 combinations of 4 positions --- and finds it deeply rugged: 92 prcetn of variants are near-zero fitness, there are 30 local peaks, and you can't get to the global optimum by greedy hill climbing. Sarkisyan et al. (2016) does the same for GFP, but across the full protein --- 52,000 variants spread across 239 residues rather than concentrated at 4 sites. Sparse, moderate epistasis, a fitness threshold where things either glow or don't. These two papers are the datasets everyone trains and tests on for the next decade.

Act 2: ML Enters the Lab (2019--2021)
With maps of the landscape in hand, the question becomes: can machine learning navigate it better than random walk? Wu et al. (2019) says yes, and names the approach MLDE. The workflow is simple: label ~100 variants, train a model, predict all 160k GB1 variants, test the top ones, repeat. One-hot encoding plus an ensemble beats greedy directed evolution.
Wittmann et al. (2021) immediately asks: what are the levers? They run a rigorous ablation on GB1 and find the biggest lever isn't the model or the encoding --- it's the training set. If your training data is 92percent holes (near-zero fitness), the model learns to predict near-zero fitness everywhere. Their fix, ftMLDE, uses zero-shot predictors (EVmutation, MSA Transformer) to enrich the training set with functional variants before fitting any surrogate. 81$\times$ improvement over single-step greedy. The encoding comparison is also here: ESM-1b helps but not dramatically; one-hot is competitive throughout.
In parallel, Biswas et al. (2021) makes the case that PLM embeddings shine specifically when you have very few labels --- 24 variants, not 96. The language model's evolutionary prior substitutes for experimental data you don't have.


  Act 3: The Greedy Baseline Wins (2020--2022)
While ML people are building sophisticated surrogates, Sinai et al. (2020) builds FLEXS and shows something uncomfortable: a deliberately simple algorithm, AdaLead, beats BO, RL, and generative models across most benchmarks. AdaLead just picks the best sequences seen so far, mutates and recombines them, filters with a surrogate, and repeats. The implication: before you trust that your fancy BO is working, you need to beat AdaLead. Most papers don't.
Ren et al. (2022 --- PEX) adds one principled idea: stay close to wild-type. Sequences far from the training distribution will fool your surrogate. Add a proximity penalty to the acquisition function. This consistently beats standard BO and AdaLead. The key biological insight is that most real engineering wins involve only a handful of mutations, so constraining search to proximal variants loses little and gains reliability.
Stanton et al. (2022 --- LaMBO) goes the other direction --- into latent space. Train a denoising autoencoder on sequences, do BO with a deep kernel learning GP in the latent space, gradient-ascend the acquisition function. Now you can propose sequences outside the measured pool. Validated in vitro on red fluorescent proteins.

Act 4: Language Models as Feature Extractors (2021--2023)
The protein language model wave arrives. ESM1b (Rives et al. 2021) shows that a 650M parameter transformer trained on 250M sequences learns structural contacts, mutational effects, and conservation implicitly. ESM2 (Lin et al. 2023) scales this to 15B parameters and adds ESMFold. ProtTrans (Elnaggar et al.) brings the NLP BERT/T5 architectures over to proteins.
The key tension that emerges: on combinatorial landscapes like GB1, ESM2 embeddings do not beat one-hot encoding. This gets confirmed independently by Wittmann, ALDE, and the BIBM 2024 paper. The intuition is that for 4-site combinatorics, the language model's evolutionary prior is noise --- the task is interpolation, not generalization. ESM2 helps more when you need to generalize across sequence space, or when you have almost no data.

Act 5: The Current Frontier (2024)
Two papers define where the field is now and are your most important reads:
ALDE (Yang et al. 2024) is the methodological paper. It benchmarks every combination of encoding $\times$ model $\times$ acquisition on GB1 and TrpB, and finds: DNN ensemble + one-hot + Thompson Sampling wins. ESM2 doesn't help. Frequentist uncertainty (ensemble spread) beats Bayesian GP uncertainty. Then they take this into the wet lab --- 5 active-site residues of a protoglobin enzyme, 3 rounds, 480 variants, 12% to 93% yield.
EVOLVEpro (Jiang et al. 2024, Science) is the engineering paper. Same general idea --- PLM + lightweight regression + active selection --- but applied to six wildly different proteins (CRISPR nuclease, RNA polymerase, antibody, integrase) with as few as 16 variants per round. Up to 100-fold improvement in 3--4 rounds. This is the closest published analog to your work.
Kirjner et al. (2024 --- ICLR) is the one most people miss: they smooth the fitness landscape before optimizing it. Instead of worrying about the acquisition function, they make the surrogate itself easier to navigate via graph-based Tikhonov regularization + Gibbs sampling. Tested directly on GFP. If you're comparing numbers on the Sarkisyan dataset, this is your benchmark.

------
-. Dry study datasets
Big datasets that enabled computattional
Dataset 
| Sarkisyan et al. | 2016 | avGFP (~51k variants, up to 15 mut) | --- | --- | --- | $\checkmark$ | $\times$ |

GFP NOTE Baselines. We choose a representative set of prior works that evaluated on GFP and AAV: GFlowNets
...
GFP / avGFP (Sarkisyan et al. 2016 --- ~52,000 variants, full protein, 239 residues, 1--15 mutations)
Much less thoroughly benchmarked for BO specifically. The dataset is used differently --- mostly as an oracle for generative methods and structure-based design, not for pool-based MLDE. Papers that touch it: PEX (Ren 2022) uses it as one of several benchmarks for proximal exploration; Kirjner 2024 tests landscape smoothing on it; LaMBO uses fluorescent proteins but with RFPs, not avGFP. Gonzalez Somermeyer 2022 trained neural networks on it (as an oracle for their genetic algorithm) but only one-hot, no PLMs. Biswas 2021 includes GFP but in a different regime.




GFP / avGFP (Sarkisyan 2016 --- ~52,000 variants, fluorescence)
PaperMethodSuccess MetricBudgetNotesBrookes \& Listgarten 2018 (DbAS)VAE generative modelFitness of generated sequences vs oracleNot fixed --- generativeNot pool-based; proposes novel sequencesBrookes et al. 2019 (CbAS)VAE + importance weightingMax predicted fitness; fraction above thresholdNot fixed --- generativeSame --- generative paradigm, not pool-basedPEX Ren et al. 2022Proximal BO, proximity-regularised acquisitionNormalized max fitness achieved; NDCG score384--960 oracle callsGFP is one of 8 DMS datasets; pool-based on SarkisyanGonzalez Somermeyer et al. 2022NN oracle + genetic algorithmWet-lab fluorescence of generated variantsNot fixed --- generativeGenerative oracle paradigm; no surrogate BOKirjner et al. 2024Smoothed BO + Gibbs samplingNormalized max fitness achieved; comparison to baselines256--1,024 oracle callsDirectly on Sarkisyan pool; best GFP BO numbers in literatureLaMBO Stanton 2022DAE + DKL-GPPareto front on Stability + SASANot fixedUses RFPs, not avGFP --- not directly comparable


Key landscape findings --- the numbers you need

At least 75% of single mutations are deleterious to fluorescence
9.4% of single mutations confer >5-fold decrease
Half of all 4-mutant variants are completely non-fluorescent
Negative epistasis affects up to 30% of all genotypes --- the joint effect of multiple mutations is worse than the sum of their individual effects
Positive epistasis is rare, ~1% of pairs
Epistasis was found among 96% of mutations with weak individual effects --- mutations that individually barely matter interact epistatically with each other

| Wu et al. (GB1) | 2016 | GB1 (160k variants, 4-site) | --- | --- | --- | $\checkmark$ | $\times$ |
GB1 (Wu et al. 2016 --- 149,361 variants, 4-site combinatorial, IgG binding)
This is the most-studied benchmark by far. Nearly everything has been tested here: Wu 2019 established MLDE on it, Wittmann 2021 ran the exhaustive encoding $\times$ model $\times$ training set ablation on it, Biswas 2021 tested low-N PLM engineering on it, ALDE 2024 ran all their computational benchmarks on it, Greenberg 2023 benchmarked UQ methods on it, and the BIBM 2024 paper tested embedding-space BO on it. AdaLead, CbAS, DbAS, DyNA-PPO have all been evaluated here too. It's saturated. The landscape properties are well-characterized: highly epistatic, 92% holes, 30 local optima, indirect paths to the global optimum.

GB1 (Wu et al. 2016 --- 149,361 filtered variants, binding affinity)
PaperMethodSuccess MetricBudgetNotes
Wu et al. 2019MLDE, one-hot + ensembleMean fitness of top-96 predicted; fraction above threshold96--480 labelsFirst use of GB1 as oracle
Wittmann et al. 2021MLDE / ftMLDE, one-hot + various encodingsGlobal optimum hit rate (fraction of 50 replicates finding the maximum); average max fitness96--480 labels81$\times$ improvement over single-step greedy
Biswas et al. 2021ESM1b embeddings + linear regressionAverage fitness of top predictions24--384 labelsLow-N focus; GB1 one benchmark among severalALDE Yang et al. 2024DNN ensemble + one-hot + Thompson SamplingAverage max fitness achieved; fraction reaching global optimum120--2,016 total (multiple rounds of 120--480)Matched budget comparison across encoding $\times$ model $\times$ acquisitionLi et al. 2024MLDE / ftMLDE / ALDE / ftALDE, one-hot + boosting ensembleAverage max fitness achieved; fraction reaching global optimum120--2,016 totalGB1 is 1 of 16 landscapes; same metrics as ALDEGreenberg et al. 2023GP, MC-Dropout, ensemble, conformal predictionUQ calibration metrics (ECE, CRPS); downstream active learning fitness96--384 labelsNot an optimization study; evaluates UQ qualityBIBM 2024BO in ESM2 latent space vs one-hotMax fitness achieved; computational costVariousGP surrogate; embedding-space BO
What's comparable on GB1: Wittmann, ALDE, and Li et al. all report global optimum hit rate and average max fitness at matched budgets (480 total samples is the common reference point Li et al. use). Wu 2019 uses a slightly different metric (top-96 mean) so not directly comparable but directionally consistent. Greenberg is not an optimization paper and cannot be compared.
Critical caveat: Wittmann uses 50 replicates, ALDE and Li et al. use 50 replicates, Wu 2019 uses fewer. Comparable only within similar replicate counts.



| Johnston et al. | 2024 | TrpB (160k variants, 4-site) | --- | --- | --- | $\checkmark$ | $\times$ |


PaperMethodSuccess MetricBudgetNotesALDE Yang et al. 2024DNN ensemble + one-hot + Thompson SamplingAverage max fitness achieved; fraction reaching global optimum120--2,016 totalComputational benchmark only; same setup as GB1 armLi et al. 2024MLDE / ftMLDE / ALDE / ftALDEAverage max fitness achieved; fraction reaching global optimum120--2,016 total10 TrpB landscapes; dominant dataset in paper


| Romero \& Arnold | 2009 | Multiple proteins (review) | --- | --- | --- | --- | $\times$ |


| Arnold | 1998 | Various enzymes | Random mutagenesis | --- | --- | $\checkmark$ | $\times$ |


| Stemmer | 1994 | $\beta$-lactamase | DNA shuffling | --- | --- | $\checkmark$ | $\times$ |
...
% #endregion
----------------------------------
% #region [DRAFT] Methodology notes
Methodology
* Embedding/main model
* Surrogate model
* Acquisition function
* Evaluation metric
* Main loop (full BO, mix)
...
Directed evolution~\cite{Romero2009}\textbf{[TODO: cite Packer2015]} mimics natural selection in the laboratory:
...
saturation mutagenesis~\textbf{[TODO: cite Stemmer1994]}, and the best-performing variants are identified
...
The introduction of deep mutational scanning (DMS)~\textbf{[TODO: cite Fowler2014]} partially addressed
...
Physicochemical encodings such as AAIndex~\textbf{[TODO: cite Kawashima2000]} or the Georgiev reduced
alphabet~\textbf{[TODO: cite Georgiev2009]} map each amino acid to a small number of empirically measured
...
ensembles~\textbf{[TODO: cite Lakshminarayanan2017]}.
...
uncertainty estimates via ensemble disagreement~\textbf{[TODO: cite Lakshminarayanan2017]};
...
Upper confidence bound (UCB; select by $\hat{\mu} + \beta\hat{\sigma}$)~\textbf{[TODO: cite Srinivas2010]}
...
The ProteinGym benchmark~\textbf{[TODO: cite Notin2023ProteinGym]} provides a comprehensive evaluation of
...
The ProteinGym benchmark~\textbf{[TODO: cite Notin2023ProteinGym]} aggregates DMS data for 217 proteins and
...
zero-shot benchmarking~\textbf{[TODO: cite Notin2023ProteinGym]} and MLDE studies~\cite{Wu2019, Yang2025}.
...
Each amino acid substitution is encoded using a selected subset of AAIndex~\textbf{[TODO: cite Kawashima2000]}
...
An exact GP with a radial basis function (RBF) kernel is trained using GPyTorch~\textbf{[TODO: cite Gardner2018]}.
...
but different random initialisations~\textbf{[TODO: cite Lakshminarayanan2017]}.
