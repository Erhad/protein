...
for two reasons: (i) the map φbetween sequences X= {x1,···
,xn}and their biological function
y= {y1,···,yn}is non-convex and (ii) has sparse support in the space of possible sequences
AL, which also grows exponentially in the length of the sequence Lfor alphabet A. This map φis
...
models φ′: X →y which approximate the oracle φthat maps each sequence to its true function. We
may use these models φ′as proxies of φ, in order to generate and screen sequences in silico before
they are sent to synthesis (Yang et al., 2019; Fox et al., 2007). While a large body of work has focused
on building better local approximate models φ′on already published data (Otwinowski et al., 2018;
...
performed. Hence data is often collected in serial batches bi, comprising data Dt = {b0,···,bt}and
the problem of sequence design is generally cast as that of proposing batches so that we may find the
optimal sequence x∗
t = argmaxx∈Dt φ(x) over the course of these experiments
----------------------------------
PLAN and thinking
...
ALDE (Yang et al. 2024) is the methodological paper. It benchmarks every combination of encoding × model × acquisition on GB1 and TrpB, and finds: DNN ensemble + one-hot + Thompson Sampling wins. ESM2 doesn't help. Frequentist uncertainty (ensemble spread) beats Bayesian GP uncertainty. Then they take this into the wet lab — 5 active-site residues of a protoglobin enzyme, 3 rounds, 480 variants, 12% to 93% yield.
...
3. Relevant wet studies
Now circling back to more general methods and starting wet again more systematically 
PROGEN? (what were the others) . Mention that the field
